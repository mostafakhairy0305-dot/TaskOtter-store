# NPM Taskfile (fnm) Public Tasks

## What is this Taskfile?

This Taskfile wraps common `npm` operations and requires the **fnm stack**.
...
This Taskfile wraps common `npm` operations — installing dependencies, running scripts, auditing, and cleaning — behind consistent, cross-platform task commands. It delegates Node.js installation to **fnm**, keeping each concern in its own taskfile.
...
Before using npm tasks on a fresh machine, install Node.js via fnm:
...
This delegates to `task fnm:node:install`.
...
| `node:setup`      | Optional `NODE_VERSION`                    | Install Node.js and npm via fnm.                    |
