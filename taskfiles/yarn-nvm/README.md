# Yarn Taskfile (nvm) Public Tasks

This Taskfile wraps common modern Yarn project work while reusing the **nvm** Node.js loading model as the npm Taskfile. Yarn commands run through
Corepack so the project's `packageManager` field controls the Yarn version.

## Setup

This Taskfile requires the **nvm stack**. Include it in your root Taskfile:
...
| `node:setup`    | Optional `NODE_VERSION`                    | Install Node.js via nvm.                             |
